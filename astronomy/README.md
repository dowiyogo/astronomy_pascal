...
**Authors:** O.Montenbruck, T.Pfleger

# Astronomy on the Personal Computer — Floppy Disk User’s Guide

**(2nd edition 1994)**
**(instrucciones Linux 2026)**
...
| `KEPLER.PAS` | Position and velocity (two-body problem for all forms of orbit) |
...
| `Diverse` | ( A:\DIVERSE ) |
| `File` | Description |
| `TFITMOON.PAS` | Approximation of lunar coordinates by Chebyshev polynomials (without use of T_FIT_LBR) |
| `TFITSUN.PAS` | A proximation of solar coordinates by Chebyshev polynomials (without use of T_FIT_LBR) |
...
## 2.3 Linux (Debian/Ubuntu): instalación, descarga desde GitHub y compilación (Free Pascal)

Estas fuentes están organizadas en subdirectorios (por ejemplo `DE/`, `MOON/`, `PLANETS/`, `TIME/`, `UNITS/`, etc.). La carpeta `UNITS/` contiene unidades Pascal comunes usadas por varios programas; por eso, al compilar conviene **agregar esa ruta** al compilador.

### Paquetes requeridos

```bash
sudo apt update
sudo apt install -y git fp-compiler make build-essential dos2unix
```
...
### (Opcional) Convertir finales de línea DOS→Unix

Recomendado si los `.PAS` vienen con CRLF.

```bash
find . -type f \( -iname "*.pas" -o -iname "*.inc" -o -iname "*.txt" \) -print0 | xargs -0 dos2unix
```

### Compilar un programa (ejemplo)

Compila un archivo `.PAS` dentro de su carpeta, indicando dónde están las unidades compartidas:

```bash
cd PLANETS
fpc -Fu../UNITS PLANPOS.PAS
./PLANPOS
```

### Compilar “desde la raíz” (sin entrar a la carpeta)

```bash
fpc -FuUNITS PLANETS/PLANPOS.PAS
./PLANETS/PLANPOS
```

> Nota: si algún programa además usa unidades locales de su propio subdirectorio, compila desde ese subdirectorio (como en el ejemplo `cd PLANETS`) o agrega también `-Fu.`.

---

## 2.4 Linux (AlmaLinux / RHEL compatibles): instalación, descarga desde GitHub y compilación (Free Pascal)

### Paquetes requeridos

En AlmaLinux suele ser necesario habilitar EPEL para obtener `fpc`.

```bash
sudo dnf -y install epel-release
sudo dnf -y install git fpc make gcc gcc-c++ dos2unix
```
...
### Alternativa (compilar desde la raíz)
...
> Nota: si `dnf install fpc` no encuentra el paquete, verifica que EPEL esté habilitado o busca el nombre exacto con `dnf search pascal`.
